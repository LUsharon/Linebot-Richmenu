...
channel_access_tokenn = os.getenv('CHANNEL_ACCESS_TOKEN')
line_handler = WebhookHandler(os.getenv('CHANNEL_SECRET'))
configuration = Configuration(access_token=channel_access_tokenn)
...
def handle_message(event):
    ...
    with ApiClient(configuration) as api_client:
        ...
        if text == '文字':
            ...
            line_bot_api.reply_message(
                ReplyMessageRequest(
                    reply_token=event.reply_token,#$字號透過前面的索引
            ...
            url = url.replace("http", "https")
            app.logger.info("url=" + url)
            duration = 60000  # in milliseconds毫秒為單位
            line_bot_api.reply_message(
                ReplyMessageRequest(
                    reply_token=event.reply_token,
                    messages=[
                        AudioMessage(original_content_url=url, duration=duration)#音訊路徑、時長
                    ]
                )
            )
        elif text == '位置':
